...
import { InsertPlayer } from '../DB_functions/InsPlayer.ts';
...
import { correlateTeamsAndPlayers } from '../DB_functions/RecentID.ts';
...
const attribList = await AttribToArray(4);                                  // Generate and insert in table: 
...
const NameGen = await generateName();   // Generate and insert in table: 
                                        //.from('Players'); 
await InsertPlayer(NameGen,roster[i]);                  //.insert([{name: name}]);
